...
export const start = (productId, qrContainer, options) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // if (_buttonId == undefined || _buttonId === '') {
            //     const error = 'AddToCartButtonId not defined';
            //     if (events.onError) events.onError(error);
            //     reject(error);
            //     return;
            // }

            // if (!processButton(reject)) return;
            ...
            let data;
            if (isMobile() && !isIPad()) {
                _popUpWindow = window.open('', '_blank')
                data = await createIdAndLink(_organizationId, productId, options);
                _popUpWindow.location = data.link;
                _to = setTimeout(() => {
                    closePopUpWindow();
                }, 1000);
            ...
            recommendation.requestedProductId = productId;
            recommendation.options = options;
            storeRecommendationData(recommendation.id);
...
const createIdAndLink = (organizationId, productId, options) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const linkData = await createDynamicLink(organizationId, measurementData.measurementId, productId, options);
            resolve({
...
const installButtonCheck = async () => {

    _organizationInfo = await loadOrganizationInfo();
    console.log(organizationInfo);
    //_buttonId = 'cart'; // organizationInfo.buttonId;
    //reject(`AddToCartButton with ID or CSS class "${_buttonId}" not found.`);
    //return false;
}

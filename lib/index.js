...
/**
 * @param {string|[string]} productId - Single productId or a list of productIds to get recommendations for
 ...
 */
export const start = (productId, qrContainer, useMockData) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (isMobile() && !isIPad()) {
                ...
                data = await createIdAndLink(_organizationId, productId);
...
const createIdAndLink = (organizationId, productId) => {
    return new Promise(async (resolve, reject) => {
        try {
            const measurementData = await createMeasurement(organizationId);
            const linkData = await createLink(organizationId, measurementData.measurementId, productId);
            resolve({

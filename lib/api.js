...
export const loadOrganizationInfo = () => {

    return new Promise(resolve => {
        resolve({
            cartButtonIdentifierType: "CLASS",
            cartButtonId: "cart",
            cartButtonClass: "button",
            cartButtonClassIndex: 0
        })
    });

    const request = new Request(`${_baseUrl}/organizations/${_organizationId}`)
        .get()
        .json()
...
export const createShortDynamicLink = async (organizationId, measurementId, productId, options) => {
    const longDynamicLink = createDynamicLink(organizationId, measurementId, productId, options);
...
export const sendUsageAcknowledgement = () => {
    return new Promise(resolve => {

...
export const getRecommendations = async (organizationId, measurementId) => {
    if (isMobile()) {
        try {
            const recommendations = await getRecommendationsMobile(measurementId);
            return processRecommendationData(recommendations);
        } catch (e) {
            throw new Error('Loading recommendations on mobile failed.');
        }
    } else {
        try {
            ...
            throw new Error('Loading recommendations from socket failed.');
        }
    }
}
